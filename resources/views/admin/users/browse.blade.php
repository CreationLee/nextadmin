...
@section('page_header')
    <h1 class="page-title">
        ...
        @if (AdminFacades::can('add_'.$dataType->name))
                        ...
                        <table id="dataTable" class="table table-hover">
                            ...
                            @foreach($dataTypeContent as $data)
                                <tr>
                                    ...
                                    <td>
                                        <img src="@if( strpos($data->avatar, 'http://') === false && strpos($data->avatar, 'https://') === false){{ AdminFacades::image( $data->avatar ) }}@else{{ $data->avatar }}@endif" style="width:100px">
                                    </td>
                                    <td>{{ $data->role ? $data->role->display_name : '' }}</td>
                                    <td class="no-sort no-click">
                                        @if (AdminFacades::can('delete_'.$dataType->name))
                                            <div class="btn-sm btn-danger pull-right delete" data-id="{{ $data->id }}" id="delete-{{ $data->id }}">
                                                <i class="voyager-trash"></i> Delete
                                            </div>
                                        @endif
                                        @if (AdminFacades::can('edit_'.$dataType->name))
                                            <a href="{{ route('admin.'.$dataType->slug.'.edit', $data->id) }}" class="btn-sm btn-primary pull-right edit">
                                                <i class="voyager-edit"></i> Edit
                                            </a>
                                        @endif
                                        @if (AdminFacades::can('read_'.$dataType->name))
